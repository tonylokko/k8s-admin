...
# 6. VPC resources (subnets, SGs, route tables, IGW)
if [[ -n "$VPC_ID" ]]; then
    ...
    # First, revoke all rules to break cross-references between security groups
    ...
    if [[ -n "$SG_IDS" ]]; then
        echo "Revoking security group rules to break cross-references..."
